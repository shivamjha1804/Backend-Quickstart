...
if [[ "$BACKUP_DB" == true ]]; then
    ...
    if docker-compose -f "$COMPOSE_FILE" exec -T postgres pg_dump -U "$DB_USERNAME" "$DB_NAME" > "$BACKUP_FILE" 2>/dev/null; then
        print_success "Database backup created: $BACKUP_FILE"
    else
        print_warning "Database backup failed (this is normal for first deployment)"
    fi
